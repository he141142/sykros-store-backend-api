...
public interface RoleRepository extends CrudRepository<Role,Long> {
   ...
   @Query(value = "select r.id, r.name,r.amount,r.authority from Role r " +
           "inner join user_role ur " +
           "on ur.role.id = r.id " +
           "inner join Account ac on ur.account.id = ac.id where accountId, nativeQuery = true");
   Stream<Role> getRoleAndAccount( Long accountId);
